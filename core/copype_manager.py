...
class CopypeManager:
    ...
    def create_winpe_workspace_direct(self, architecture: str, workspace_dir: Path,
                                      progress_callback: Optional[Callable] = None) -> Tuple[bool, str, Path]:
        """
        直接在指定目录创建WinPE工作空间（不创建WinPE子目录）

        Args:
            architecture: 架构 (amd64, x86, arm64)
            workspace_dir: 工作空间目录（直接输出到此目录）
            progress_callback: 进度回调函数
        ...
        """
        try:
            self.logger.info(f"开始创建WinPE工作空间（直接模式）: {architecture}")

            # 运行copype，它会创建WinPE子目录
            ...
            # 将WinPE子目录的内容移动到workspace_dir根目录
            winpe_subdir = workspace_dir / "WinPE"
            if success and winpe_subdir.exists():
                self.logger.info("移动WinPE文件到目标目录...")
                try:
                    # 移动WinPE子目录中的所有内容到workspace_dir
                    for item in winpe_subdir.iterdir():
                    ...
                    # 删除空的WinPE子目录
                    winpe_subdir.rmdir()
                    self.logger.info("WinPE文件移动完成")

                except Exception as e:
                    self.logger.error(f"移动WinPE文件失败: {str(e)}")
                    success = False
                    message = f"移动WinPE文件失败: {str(e)}"

            # WinPE内容现在直接在workspace_dir中
            winpe_dir = workspace_dir

            if success:
                self.logger.info(f"WinPE工作空间创建成功: {winpe_dir}")
                return True, message, winpe_dir
            else:
                error_msg = f"WinPE工作空间创建失败: {message}"
                self.logger.error(error_msg)
            ...
            self.logger.error(error_msg)
            return False, error_msg, workspace_dir
